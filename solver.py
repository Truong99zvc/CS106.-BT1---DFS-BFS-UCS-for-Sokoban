...
def breadthFirstSearch(gameState):
    ...
    frontier = collections.deque([[startState]]) #Tạo một hàng đợi frontier là deque chứa danh sách các trạng thái ban đầu
    exploredSet = set() #Tạo một tập rỗng để lưu trữ các trạng thái đã được duyệt qua
    actions = collections.deque([()]) #Tạo một hàng đợi actions là deque chứa danh sách các hành động tương ứng với mỗi trạng thái trong frontier
    temp = [] #Tạo một list rỗng để lưu trữ lịch sử các hành động dẫn đến mục tiêu
    ### CODING FROM HERE ###
    while frontier: #Lặp khi frontier không rỗng
        node = frontier.popleft() #Lấy ra trạng thái đầu tiên từ frontier
        node_action = node[-1][-1] #Lấy ra danh sách các hành động tương ứng với trạng thái hiện tại
        ...
        if node[-1][0:2] not in exploredSet: #Kiểm tra xem trạng thái hiện tại đã được duyệt chưa
            exploredSet.add(node[-1][0:2]) #Nếu chưa, thêm trạng thái vào tập exploredSet
            for action in legalActions(node[-1][0], node[-1][1]): #Lấy danh sách các hành động hợp lệ từ trạng thái hiện tại
                ...
                frontier.append(node + [(newPosPlayer, newPosBox, node_action + (action[-1],))]) #Thêm trạng thái mới vào frontier với danh sách hành động mới
